...
function Get-Xcyclopedia {
    ...
    param (
        ...
        [bool]$export_ssdeep_list        = $false,   # Export ssdeep results to a ssdeep-compatible csv file
        [bool]$export_ssdeep_list_with_md5 = $false, # Include MD5 with ssdeep file export
        [bool]$get_sigcheck              = $false,  # Use Sigcheck (Sysinternals) to obtain additional file signatures and PE metadata.
        [bool]$get_virustotal            = $false,  # Use Sigcheck (Sysinternals) to obtain VirusTotal detection ratio.
        [bool]$accept_virustotal_tos     = $false,  # Accept VirusTotal's Terms of Service
        [string]$path_to_file_arg1       = $null,   # This filepath will be provided as an argument to each binary
        [string]$path_to_file_arg2       = $null,   # This filepath will be provided as an argument to each binary
        [bool]$convert_to_csv            = $true    # CSV export is enabled by default
    )
...
function Get-FileList {
    ...
    if (-not $file_extension -is [array]) {
    ...
    $extensions = $file_extension -join ', '
    
    Write-Host "Directory listing complete ($file_count file(s) with extension(s) $extensions found)"
