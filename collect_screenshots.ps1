...
.\xcyclopedia\script\Get-Xcyclopedia.ps1 -save_path "C:\Users\Matt\source\repos\OSVisualInstruct\output" -take_screenshots $true -minimize_windows $true -target_file_extension ".exe",".lnk" -execute_files $true -target_path_recursive "$env:APPDATA\Microsoft\Windows\Start Menu\Programs","$env:ProgramFiles","${env:ProgramFiles(x86)}","$env:windir\system32"
